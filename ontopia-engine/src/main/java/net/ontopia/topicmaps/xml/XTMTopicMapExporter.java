...
public class XTMTopicMapExporter extends AbstractTopicMapExporter {
  ...
  protected void writeInstanceOf(TopicIF topic, DocumentHandler dh)
    ...
    if (!types.isEmpty()) {
      ...
      while (iter.hasNext()) {
        dh.startElement(INSTANCE_OF, EMPTY_ATTR_LIST);
        writeTopicRef((TopicIF) iter.next(), dh);
        dh.endElement(INSTANCE_OF);
      }
    }
  }
  ...
  protected void writeTopicRef(TopicIF topic, DocumentHandler dh)
      throws SAXException {
    atts.clear();
    atts.addAttribute(XLINK_HREF, CDATA, "#" + getElementId(topic));
  ...
  protected void writeSubjectIdentity(TopicIF topic, DocumentHandler dh)
    ...
    if (outputIdentities) {
      ...
      if (!subjects.isEmpty()) {
        ...
        if (it.hasNext()) { // NOTE: we only pick out the first one
          ...
          if (notation != null && URI.equals(notation)) {
            atts.clear();
            atts.addAttribute(XLINK_HREF, CDATA, subject.getExternalForm());
      ...
      if (!subinds.isEmpty()) {
        ...
        while (it.hasNext()) {
          ...
          if (notation != null && URI.equals(notation)) {
            atts.clear();
            atts.addAttribute(XLINK_HREF, CDATA, getSubjectIndicatorRef(
                                                                            topic, indicator));
      ...
      if (export_srclocs && !srclocs.isEmpty()) {
        ...
        while (it.hasNext()) {
          ...
          if (notation != null && URI.equals(notation)) {
            atts.clear();
            atts.addAttribute(XLINK_HREF, CDATA, srcloc.getExternalForm());
            dh.startElement(SUBJECTINDICATORREF, atts);
            dh.endElement(SUBJECTINDICATORREF);
          } else
    ...
    if (reified != null) {
      atts.clear();
      atts.addAttribute(XLINK_HREF, CDATA, "#" + getElementId(reified));
      dh.startElement(SUBJECTINDICATORREF, atts);
      dh.endElement(SUBJECTINDICATORREF);
    }
    ...
    if (ObjectUtils.equals(variant.getDataType(), DataTypes.TYPE_URI)) {
			...
			if (varloc != null) {
				...
				if (notation != null && URI.equals(notation)) {
					// Write resourceRef
					atts.clear();
					atts.addAttribute(XLINK_HREF, CDATA, varloc.getExternalForm());
					dh.startElement(RESOURCEREF, atts);
					dh.endElement(RESOURCEREF);
				} else
			...
			if (ObjectUtils.equals(occr.getDataType(), DataTypes.TYPE_URI)) {
				...
				if (occloc != null) {
					...
					//! if (notation != null && notation.equals("URI")) {
						atts.clear();
						atts.addAttribute(XLINK_HREF, CDATA, occloc.getExternalForm());
						dh.startElement(RESOURCEREF, atts);
						dh.endElement(RESOURCEREF);
					//! } else
